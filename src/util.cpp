...
#else
    const char* pszModule = "MasterMint";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MasterMint
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MasterMint
    // Mac: ~/Library/Application Support/MasterMint
    // Unix: ~/.MasterMint
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MasterMint";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "mastermint.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "mastermintd.pid"));
