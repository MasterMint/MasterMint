...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("MasterMint"))
        return false;
...
{
    // Convert MasterMint:// to MasterMint:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("MasterMint://"))
    {
        uri.replace(0, 12, "MasterMint:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MasterMint.lnk";
}
...
{
    return GetAutostartDir() / "MasterMint.desktop";
}

bool GetStartOnSystemStartup()
{
    boost::filesystem::ifstream MasterMintFile(GetAutostartFilePath());
    if (!MasterMintFile.good())
        return false;
    // Scan through file for "Hidden=true":
    std::string line;
    while (!MasterMintFile.eof())
    {
        getline(MasterMintFile, line);
        if (line.find("Hidden") != std::string::npos &&
            line.find("true") != std::string::npos)
            return false;
    }
    MasterMintFile.close();
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        boost::filesystem::ofstream MasterMintFile(GetAutostartFilePath(), std::ios_base::out|std::ios_base::trunc);
        if (!MasterMintFile.good())
            return false;
        // Write a bitcoin.desktop file to the autostart directory:
        MasterMintFile << "[Desktop Entry]\n";
        MasterMintFile << "Type=Application\n";
        MasterMintFile << "Name=MasterMint\n";
        MasterMintFile << "Exec=" << pszExePath << " -min\n";
        MasterMintFile << "Terminal=false\n";
        MasterMintFile << "Hidden=false\n";
        MasterMintFile.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MasterMint-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  MasterMint-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MasterMint-Qt"));
